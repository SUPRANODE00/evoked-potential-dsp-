...
class SAPReserveProcessFilter:
    ...
    def process_telemetry(self, raw_value, delta_time):
        # Vector 1: PLL Phase Tracking
        expected_phase = 2 * np.pi * self.target_freq * delta_time
        phase_error = raw_value - np.sin(self.phase_accumulator)
        self.phase_accumulator += expected_phase + (self.pll_loop_gain * phase_error)

        # Vector 2: Attenuation & Low-Pass Cutoff
        self.filtered_state = (self.alpha * raw_value) + (
            (1 - self.alpha) * self.filtered_state
        )
...
if __name__ == "__main__":
    ...
    sample_rate = 100  # Hz
    target_freq = 5  # Hz
    filter_node = SAPReserveProcessFilter(
        sample_rate=sample_rate, target_freq=target_freq
    )

    print("\nProcessing simulated packet stream:")
    hdr = (
        f"{'Packet':<8}{'Raw Val':<10}"
        f"{'Delta t':<10}{'Filtered Out':<15}{'Phase Accum':<12}"
    )
    print(hdr)
    print("-" * 55)
    ...
    for i in range(1, 11):
        raw_val = np.sin(2 * np.pi * target_freq * (i * base_dt)) + np.random.normal(
            0, 0.2
        )
        jittered_dt = base_dt + np.random.normal(0, 0.002)
        output, phase = filter_node.process_telemetry(raw_val, jittered_dt)
        print(
            f"{i:<8}{raw_val:<10.4f}{jittered_dt:<10.4f}{output:<15.4f}{phase:<12.4f}"
        )
